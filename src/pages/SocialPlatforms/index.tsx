  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-500">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 font-outfit">{t('sidebar.socialPlatforms')}</h1>
          <p className="mt-1 text-sm text-slate-500">{t('socialPlatformsPage.subtitle')}</p>
        </div>
        <button
          type="button"
          onClick={openCreate}
        ...
        <table className="w-full border-collapse text-start">
          <thead>
            <tr className="border-b border-slate-100">
              <th className="px-6 py-4 text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.colName')}</th>
              <th className="px-6 py-4 text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.colInput')}</th>
              <th className="px-6 py-4 text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.colBaseUrl')}</th>
              <th className="px-6 py-4 text-[11px] font-bold uppercase text-slate-400">{t('common.status')}</th>
              <th className="px-6 py-4 text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.colUpdated')}</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50/60">
                ...
                <td className="max-w-[200px] truncate px-6 py-4 font-mono text-xs text-slate-500">{row.base_url ?? '—'}</td>
                ...
                <td className="px-6 py-4 text-xs text-slate-500">{row.updated_at ? formatDate(row.updated_at) : '—'}</td>
        ...
        <SheetContent className="w-full max-w-lg overflow-y-auto sm:max-w-lg">
          ...
          {editing ? (
            <form onSubmit={(e) => void submitUpdate(e)} className="space-y-4">
              ...
              <label className="space-y-1 text-sm">
                <span className="text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.inputType')}</span>
                <select
              ...
              <label className="space-y-1 text-sm">
                <span className="text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.baseUrl')}</span>
                <input
                  value={form.base_url}
              ...
              <label className="space-y-1 text-sm">
                <span className="text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.placeholderEn')}</span>
                <input
              ...
              <label className="space-y-1 text-sm">
                <span className="text-[11px] font-bold uppercase text-slate-400">{t('socialPlatformsPage.placeholderAr')}</span>
                <input
                  dir="rtl"
